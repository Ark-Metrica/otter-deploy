...
timeout 5 python3 /bin/wg-request --port "${PORT}" --private-key $(cat /tmp/peer_A.key) $(cat /tmp/peer_A.pub) pipe.0x3.ca > "/etc/wireguard/${IFACE}.conf" 2>/dev/null
wg-quick down "${IFACE}" 2>&1 >/dev/null
wg-quick up "${IFACE}" 2>&1 >/dev/null
systemctl enable "wg-quick@${IFACE}" 2>&1 >/dev/null
